...
def _get_weightbot_data():
    ...
    with open(path.join(path.dirname(path.realpath(__file__)), '..', 'weight-data/weightbot_data.csv')) as wb_file:
        ...
        return [{'total': float(x.split(',')[1].strip()), 'date': datetime.strptime(x.split(',')[0], '%Y-%m-%d')} for x in wb_data]


def _get_mfp_data():
    print('Getting MyFitnessPal data (from www.myfitnesspal.com)')
    login_req = post('https://www.myfitnesspal.com/account/login', {'username': 'taylanaydinli', 'password': 'hedehodo2013'}, verify=False)
    session_id = login_req.cookies['_session_id']

    resp = get('http://www.myfitnesspal.com/reports/results/progress/1/365.json?report_name=Weight', cookies=dict(_session_id=session_id)).json()
    ...
    return [{'total': x['total'], 'date': datetime.strptime(x['date'].replace('/', '-'), '%Y-%m-%d')} for x in weight_data]
